...
function serializeMessage(m, sock) {
  try {
    ...
    // Private chat: remoteJid sendiri yang berformat @lid (kalau ada),
    // sedangkan remoteJidAlt adalah versi @s.whatsapp.net-nya (kebalikan grup).
    let senderLid = isGroup ? participant2 : (key.remoteJid?.endsWith('@lid') ? key.remoteJid : key.remoteJidAlt);
