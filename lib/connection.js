...
// ─── Status pairing untuk web (pair.html) ────────────────────
// Disimpan di global supaya bisa dibaca oleh server.js (Express)
// tanpa perlu import silang yang bikin circular dependency.
global.pairingStatus = global.pairingStatus || {
  qrDataUrl: null, // QR dalam bentuk data:image/png;base64,...
  pairingCode: null, // Kode pairing 8 digit (format XXXX-XXXX)
  connected: false,
  updatedAt: null,
};

function resetPairingStatus() {
  global.pairingStatus.qrDataUrl = null;
  global.pairingStatus.pairingCode = null;
  global.pairingStatus.updatedAt = Date.now();
}
...
async function connectToWhatsApp(folder = 'session') {
  ...
  if (!sock.authState.creds.registered && config.type_connection.toLowerCase() == 'pairing') {
    ...
    // Publish ke pair.html juga, biar bisa dilihat lewat browser
...
_Website: autoresbot.com/contact_`;
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (qr && config.type_connection.toLowerCase() === 'qr') {
      ...
      // Simpan juga sebagai data URL untuk ditampilkan di pair.html
      try {
        global.pairingStatus.qrDataUrl = await QRCode.toDataURL(qr, { margin: 1, scale: 8 });
        global.pairingStatus.pairingCode = null;
        global.pairingStatus.updatedAt = Date.now();
      } catch (e) {
        console.log(chalk.redBright(`Gagal generate QR untuk web: ${e.message}`));
      }
    ...
    if (connection === 'open') {
      ...
      if (folder === 'session') {
        global.pairingStatus.connected = true;
        global.pairingStatus.qrDataUrl = null;
        global.pairingStatus.pairingCode = null;
        global.pairingStatus.updatedAt = Date.now();
      }
    ...
    if (connection === 'close') {
      ...
      if (folder === 'session') {
        global.pairingStatus.connected = false;
      }
