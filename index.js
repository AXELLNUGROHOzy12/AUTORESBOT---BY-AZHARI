...
if (major < 20) {
  ...
  try {
    ...
    // Jalankan web server (pair.html) lebih dulu, terpisah dari koneksi WA,
    // supaya halaman pairing tetap bisa diakses walau bot masih proses konek.
    try {
      const { startWebServer } = await import('./server.js');
      startWebServer();
    } catch (error) {
      console.log('Error starting web server:', error.message);
      logError(error, { plugin: 'process', command: 'startWebServer' });
    }
